...
public class NameListView extends BorderPane {
	...
	public NameListView(PhoneBook phoneBook) {	
		...
		Button removeName = new Button("Remove Name");
		removeName.setOnAction(e -> removeName());
		
		addNumberButton = new Button("Add number");
		addNumberButton.setOnAction(e -> addNumber());
		
		Button removeNumber = new Button("Remove Number");
		removeNumber.setOnAction(e -> removeNumber());
		
		HBox buttonBox = new HBox();
		buttonBox.setSpacing(5);
		buttonBox.setPadding(new Insets(10, 10, 10, 10));
		buttonBox.getChildren().addAll(numbersLabel, addButton, addNumberButton, removeName, removeNumber);
		setBottom(buttonBox);
	...
	private void removeName() {
		clearSelection();
		String labels = "Name";
		Optional<String> result = Dialogs.oneInputDialog("Remove phone book entry", 
				"Enter the name and press Ok.", labels);
		if (result.isPresent()) {
			String inputs = result.get();
			if (inputs != null) {
				boolean success = phoneBook.remove(inputs);
				if(success) {
					obsList.setAll(phoneBook.names());
					select(inputs);
				} else {
	...
	private void removeNumber() {
		...
		if (index != -1) {
			String name = obsList.get(index);
			Optional<String> result = Dialogs.oneInputDialog("Remove phone number to " + name, "Enter the number to remove", "Number" );
			if (result.isPresent()) {
				String input = result.get();
