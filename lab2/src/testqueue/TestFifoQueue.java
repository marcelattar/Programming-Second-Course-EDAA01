...
public class TestFifoQueue {
	...
	@Test
	public final void testIteratorEmptyList() {
		Iterator<Integer> itr = myIntQueue.iterator();
		try {
			itr.next();
				fail("itr.next() should give NoSuchElementException.");
	...
	public final void testIterator() {
		...
		while(itr.hasNext()) {
			temp = itr.next();
			i++;
			assertEquals("returns incorrect element", temp, i);
			
		}
		assertEquals("Checks if it has looped enough times.", i, 4);
